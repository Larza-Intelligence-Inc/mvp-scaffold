...
- **Better Auth email**: verification on sign-up (`requireEmailVerification`), password reset, magic-link sign-in, and organization invitation emails all go through Resend (`backend/src/lib/email.ts`). Frontend pages: `/check-email`, `/forgot-password`, `/reset-password`, `/magic-link`.
- **E2E proof**: `RESEND_API_KEY=… npm run test:resend --prefix backend` (Hono send) and `DATABASE_URL=… BETTER_AUTH_SECRET=… RESEND_API_KEY=… npm run test:auth-email --prefix backend` (auth email flows).
