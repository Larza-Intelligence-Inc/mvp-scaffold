...
docker compose exec api npm run db:push   # create the `users` table
```

- Web: http://localhost:3000 · Swagger: http://localhost:3001/ui · DB: localhost:5432 (app/dev/app)
- Studio GUI: `docker compose exec api npm run db:studio` → open https://local.drizzle.studio
...
- **Root Directory:** `api`  (Railway then builds `api/Dockerfile` — the production one)
...
WEB_ORIGIN   = https://${{web.RAILWAY_PUBLIC_DOMAIN}}   # for CORS, once web exists
```
The production Dockerfile runs `db:migrate` on boot, so the committed migration in
`api/drizzle/` creates the `users` table automatically on first deploy.
...
API_URL_INTERNAL   = http://${{api.RAILWAY_PRIVATE_DOMAIN}}:${{api.PORT}}
...
`DATABASE_URL`, `API_URL_INTERNAL`, `NEXT_PUBLIC_API_URL` are the exact keys compose
